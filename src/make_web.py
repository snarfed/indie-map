...
Writes site.json.gz ... output files, one per site, into three directories in
the current directory: base, full, and indie.
...
to be merged in. Each record must have at least a 'domain' property. Currently
this is three BigQuery result files:

Singular columns (~200GB)
...
See ../crawl/notes for more details.
"""
...
def make_full(sites, single_links, *extras):
    ...
    for i, site in enumerate(sites + more_sites):
        ...
        # endpoints.
        # the bigquery query that generates these has to concatenate them to make
        ...
        for name, val in endpoints.items():
            if val:
                ...
                for match in http_re.finditer(val, start + 1):
                    ...
                    start = match.start()
                urls.append(val[start:])

        tags.extend([name for name, vals in endpoints.items() if vals])
...
def make_indie(full, domains):
    """Generates and returns output JSON objects with links to IndieWeb domains.

    IndieWeb domains are just the sites in the dataset itself.

    Args:
      full: sequence of full site objects created by make_full()
      domains: set of IndieWeb domains to limit links to

    Returns:
      sequence of output JSON site objects
    """
    print('\nGenerating indie..', end='', flush=True)

    indie = [copy.deepcopy(site) for site in full if site['domain'] in domains]

    for i, site in enumerate(indie):
        if i and i % 100 == 0:
            print('.', end='', flush=True)
...
if __name__ == '__main__':
    ...
    json_dump('indie', make_indie(full, domains))
